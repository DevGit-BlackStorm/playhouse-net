...
namespace PlayHouse.Service.Api
{
    internal class AllApiSender : XApiCommonSender, IApiSender, IApiBackendSender
    {
        ...
        public async Task<JoinStageResult> JoinStage(string playEndpoint, string stageId, IPacket packet)
        {
            ...
            return new JoinStageResult(reply.ErrorCode, res.StageIdx, CPacket.Of(res.PayloadId, res.Payload,packet.MsgSeq));
        }
                                                ...
                                                IPacket joinPacket)
        {
            ...
            return new CreateJoinStageResult(
                    ...
                    res.IsCreated,
                    res.StageIdx,
                    CPacket.Of(res.CreatePayloadId, res.CreatePayload,reply.MsgSeq),
                    CPacket.Of(res.JoinPayloadId, res.JoinPayload,reply.MsgSeq)
            );
        }
    }
}
