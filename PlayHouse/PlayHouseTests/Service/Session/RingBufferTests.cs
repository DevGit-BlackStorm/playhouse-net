...
public class RingBufferTests
{
    ...
    public void Enqueue_WhenQueueIsFull_ShouldResizeBuffer()
    {
        ...
        var act = () => ringBuffer.Enqueue(0x03);
    ...
    public void WriteAndReadLargeAmountOfData()
    {
        ...
        // Assert
        ringBuffer.Count.Should().Be(largeAmountOfData.Length, "all data should be written to the buffer");

        // Act & Assert
        // 데이터를 하나씩 Read하며 원본 데이터와 비교
        for (var i = 0; i < largeAmountOfData.Length; i++)
        {
            var byteRead = ringBuffer.ReadByte();
            byteRead.Should().Be(largeAmountOfData[i],
                $"because the byte at position {i} should match the original data");
        }

        ringBuffer.Count.Should().Be(0, "all data should have been read");
    }
    ...
    public void Should_Handle_Massive_Amount_Of_Data_Correctly()
    {
        ...
        for (var i = 0; i < iterations; i++)
        {
            ...
            peekedValue.Should().Be(expectedValue,
                $"PeekInt32 should return the correct value at iteration {i} without modifying the buffer state.");
        }

        ringBuffer.Count.Should().Be(iterations * sizeof(int), "Peek should not change the buffer state.");
        ...
        for (var i = 0; i < iterations; i++)
        {
            ...
            readValue.Should().Be(expectedValue, $"ReadInt32 should return the correct value at iteration {i}.");
        }

        ringBuffer.Count.Should().Be(0, "All bytes should have been read from the buffer.");
    }
    ...
    public void Should_Throw_IndexOutOfRangeException_When_PeekInt16_After_Specific_Sequence_Of_Operations()
    {
        ...
        //Action act = () =>
        var length = 32 * 3; // 원하는 배열의 길이를 설정합니다. 예시에서는 10으로 설정했습니다.
